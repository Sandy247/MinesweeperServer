...
public class MinesweeperServer {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try (
                        ...
                        String output = handleRequest(null);
                        out.println(output);
                        for (String line = in.readLine(); line != null; line = in.readLine()) {
                            output = handleRequest(line);
                            if (output !="Bye"&&output!="Bomb") {
    ...
    private String handleRequest(String input) {
        ...
        if(input==null)
            return "Welcome to Minesweeper. Players: " + clients + " including you. Board: " + board.getSize()[1] + " columns by " + board.getSize()[0] + " rows. Type 'help' for help.";
    ...
    public static void runMinesweeperServer(boolean debug, Optional<File> file, int sizeX, int sizeY, int port) throws IOException {
        ...
        if(file.isPresent())
            board=Board.createBoard(file.get());
        else if(sizeX>0&&sizeY>0)
            board=Board.createBoard(sizeX,sizeY);
        else
            board=Board.createBoard(DEFAULT_SIZE,DEFAULT_SIZE);
